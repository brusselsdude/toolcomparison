...
export function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      ...
      tags: post.tools || [],
      authors: post.author?.name ? [post.author.name] : ['Tool Decision Engine'],
    },
...
export default function ComparisonPage({ params }) {
  ...
  const title = post.seoTitle || post.title;
  const authorName = post.author?.name || 'Tool Decision Engine';
    ...
    '@graph': [
      {
        ...
        author: {
          ...
          name: authorName,
          url: `${SITE_URL}/about/`,
          worksFor: { '@type': 'Organization', name: 'NILUS' },
        },
        publisher: { '@type': 'Organization', name: 'NILUS', url: SITE_URL },
  ...
  // FAQ schema
  const faqMatch = (post.content || '').match(/<h2>FAQ<\/h2>([\s\S]*?)$/);
  if (faqMatch) {
    const qas = [...faqMatch[1].matchAll(/<h3>(.*?)<\/h3>\s*<p>([\s\S]*?)<\/p>/g)];
    if (qas.length > 0) {
      jsonLd['@graph'].push({
        '@type': 'FAQPage',
        '@id': `${pageUrl}#faq`,
        mainEntity: qas.map(m => ({
          '@type': 'Question',
          name: m[1].replace(/<[^>]+>/g, ''),
    ...
    <>
      ...
      <section className="relative pt-28 pb-8 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6">
          {/* Breadcrumb */}
          <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-sm text-midnight-500 mb-6 flex-wrap">
          ...
          {/* Title */}
          <h1 className="font-display text-4xl md:text-5xl font-extrabold text-white leading-[1.1] tracking-tight mb-4">
            {post.title}
          </h1>

          <p className="text-lg text-midnight-300 leading-relaxed max-w-2xl mb-6">{post.description}</p>
                      ...
                      <div key={tool}>
                        <div className="flex items-center justify-between mb-1.5">
                          <span className={`text-sm font-medium ${isWinner ? 'text-white' : 'text-midnight-300'}`}>
                            {tool}{isWinner && <span className="ml-2 text-[10px] text-electric-400 font-semibold uppercase">Winner</span>}
                          </span>
